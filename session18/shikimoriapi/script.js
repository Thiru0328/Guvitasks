...
async function getAllPosts() {
  try {
    let response = await fetch("https://shikimori.one/api/doc/1.0/comments/index");
  ...
  try {
    ...
    if (postIdToUpdate) {
      ...
      let response = await fetch(
        `https://shikimori.one/api/doc/1.0/comments/update/${postIdToUpdate}`,
        {
          method: "PUT",
      ...
      let response = await fetch("https://shikimori.one/api/doc/1.0/comments/create/", {
        method: "POST",
        body: JSON.stringify(postPayload),
        headers: {
          "Content-type": "application/json; charset=UTF-8",
...
async function updatePost(postId) {
  try {
    ...
    let response = await fetch(
      `https://shikimori.one/api/doc/1.0/comments/create/${postId}`
    );
...
async function deletePost(postId) {
  try {
    let response = await fetch(
      `https://shikimori.one/api/doc/1.0/comments/destroy/${postId}`,
      {
        method: "DELETE",
      }
    );
